...
# Define the log file path and check if the header is needed
log_file = "performance_log.csv"
write_header = not os.path.exists(log_file)

# Create a file handler. RotatingFileHandler is good for production.
# It prevents log files from growing indefinitely.
handler = RotatingFileHandler(log_file, maxBytes=10_000_000, backupCount=5)

# Define the CSV format: We just want the raw message
...
if write_header:
    perf_logger.info("timestamp,method,path,status_code,process_time_ms")
...
class PerformanceLoggingMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        # Log the formatted CSV row
        perf_logger.info(log_data)
...
class CustomSummary(BaseModel):
    ...
    name: str = Field(
        description="A concise, descriptive name or title for the summary."
    )
    content: str = Field(
...
@app.post(
    "/query/stream", dependencies=[Depends(verify_api_key), Depends(verify_llm_ready)]
)
async def query_stream(payload: QueryModel):
    ...
    print(
        f"distance Threshold: {payload.distance_threshold}, maxlinks: {payload.max_results}"
    )
...
@app.post(
    ...
    response_model=SummaryReturnModel,
)
async def summarize_history(
    request: Request,
    payload: SummarizeHistoryRequest,
    ...
    try:
        ...
        if not history_messages:
            raise HTTPException(
                status_code=404, detail="No history found for this session_id."
            )
        ...
        if query:
            task_description = (
                f"create a detailed summary focused specifically on: '{query}'"
            )
        else:
            task_description = (
                "create a concise, general summary of the entire conversation"
            )

        # Invoke the chain ONCE and await the final Pydantic object
        summary_response = await summarization_chain.ainvoke(
            {"formatted_memory": formatted_memory, "task_description": task_description}
        )
